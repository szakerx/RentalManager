...
class ReservationsListState extends State<ReservationsListPage> {
  ...
  void initState() {
    super.initState();
    _reservations =
        Remote.getReservations().then((value) => _filteredReservations = value);
  }
                    ...
                    FlatButton(
                        ...
                        onPressed: () {
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (context) => ReservationsPage(parent: this,
                                      isInEditMode: true)));
                        },
                ...
                body: ListView.builder(
                    ...
                    itemBuilder: (_, index) {
                      return ListTile(
                        ...
                        title: Text(
                          "${_filteredReservations[index].rentalObject.name}: ${_filteredReservations[index].person.name} ${_filteredReservations[index].person.surname}",
                        ...
                        subtitle: Column(
                          children: [
                            Align(
                                ...
                                child: Text(
                                    "Cena: ${_filteredReservations[index].price} zł")),
                            Align(
                                alignment: Alignment.centerLeft,
                                child: Text(
                                    "Ilość gości: ${_filteredReservations[index].guestsCount + _filteredReservations[index].childrenCount}")),
                            Align(
                                alignment: Alignment.centerLeft,
                                child: Text(
                                    "Termin: ${getFormattedDate(_filteredReservations[index].startDate)} "
                        ...
                        trailing: FlatButton(
                            ...
                            height: 50.0,
                            onPressed: () {
                              Remote.deleteReservation(snapshot.data[index])
                                  .then((value) => setState(() {
                                        searching = !searching;
                                        updateList();
                                      }));
  ...
  void updateList() {
    setState(() {
      _reservations = Remote.getReservations()
          .then((value) => _filteredReservations = value);
      searching = false;
    });
  }
